...
import { setProductList,setInventoryList, setRetailorList, saveAllCategoryList, setCategoryList,replaceCart} from '../redux/appRedux'
...
export default function App(props) {
  ...
  const [retailorList,setRetailorList] = useState(null);
  ...
  useEffect(()=>{
    getCartInfo().then((token) =>{console.log("yayay",token)
        userCart = JSON.parse(token);
        console.log("userCart",userCart);
        if(userCart != null && userCart.length && state.onboard){
  ...
  useEffect(() => {
    console.log("state.userInfo",state.userInfo);
      ...
      fetch("https://www.grocyshop.in/api/v1/retailer/getAllRetailer",{
      method: "GET",
      headers: {
      ...
      .then((response)=>{
        if(!response.ok) throw new Error(response.status);
          ...
          console.log("retailorList",retailorList);
          var temp  = {};
          var productsList = [];
        ...
        //     }
          ...
          dispatch(setRetailorList(response));
          console.log(retailorList,retailorList);
        //   setRetailorList(response);
          var productsList = [];
        ...
        fetch("https://www.grocyshop.in/api/v1/product/getAllCategory",{
      method: "GET",
      headers: {
        "Content-type": "application/json; charset=UTF-8",
        'Authorization': 'Bearer ' + state.userInfo.token,
      ...
      .then((response)=>{
        if(!response.ok) throw new Error(response.status);
          ...
          console.log("response in getAll Category ",response);
          // setRetailorList(response);
          console.log("categoryList",retailorList);
          var temp  = {};
          var productsList = [];
  ...
  return (
    ...
    {(retailorList==undefined || retailorList.length<1) && 
    ...
    {retailorList && retailorList.length && retailorList.map((l, i) => (
        <View>
          {/* <Text>{JSON.stringify(l.contactNumber)}</Text> */}
      <Accordian 
                title = {l.shopName}
                retailorId = {l.contactNumber}
                data = {l.data}
